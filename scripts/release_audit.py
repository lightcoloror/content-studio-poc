...
MEDIA_SUFFIXES = {
    ".jpg", ".jpeg", ".png", ".webp", ".gif", ".mp4", ".mov", ".wav", ".mp3"
}
...
def audit_tree(root: Path) -> list[str]:
    ...
    if not fixture_manifest_path.is_file():
        ...
        for entry in manifest.get("fixtures", []):
            if entry.get("synthetic_only") is not True:
                errors.append(f"fixture is not synthetic_only: {entry.get('path')}")
            fixture = root / str(entry.get("path", ""))
            if not fixture.is_file():
                errors.append(f"manifest fixture is missing: {entry.get('path')}")
    ...
    for path, relative in iter_files(root):
        ...
        if matched:
            errors.append(f"forbidden path token {matched[0]}: {relative.as_posix()}")
        if path.suffix.casefold() in MEDIA_SUFFIXES:
            errors.append(f"media file is not allowed: {relative.as_posix()}")
            ...
            if relative.as_posix() != "scripts/release_audit.py":
                for pattern in ABSOLUTE_PATH_PATTERNS:
                    if pattern.search(text):
                        errors.append(f"personal absolute path found: {relative.as_posix()}")
                        break
            if relative.parts[:2] == ("content", "items"):
                if any(marker in text for marker in PRIVATE_CONTENT_MARKERS):
                    errors.append(f"private-domain content marker found: {relative.as_posix()}")
